...
async function setup(consent) {
  ...
  if (consent) dom.window.localStorage.setItem('cookie-consent-v2', consent);
  dom.window.eval(source);
  await new Promise(resolve => dom.window.setTimeout(resolve, 0));
  return dom;
}

test('Meta and GA stay disabled until explicit v2 consent', async () => {
  ...
  assert.equal(localStorage.getItem('cookie-consent-v2'), 'declined');
...
test('accepting consent loads both trackers and initializes the requested Pixel', async () => {
  ...
  assert.equal(localStorage.getItem('cookie-consent-v2'), 'accepted');
